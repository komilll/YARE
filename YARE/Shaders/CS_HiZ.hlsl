...
RWTexture2D<float> hiZSrc : register(u0);
RWTexture2D<float> hiZDst: register(u1);
...
void generateHiZMip0(uint3 index : SV_DispatchThreadID)
{
	...
	hiZSrc[index.xy] = depthCStoVS(depthCS);
}

[numthreads(16, 16, 1)]
void generateHiZMip(uint3 index : SV_DispatchThreadID)
{
	float a = hiZSrc.Load(int3(index.xy * 2 + int2(0, 0), 0));
	float b = hiZSrc.Load(int3(index.xy * 2 + int2(1, 0), 0));
	float c = hiZSrc.Load(int3(index.xy * 2 + int2(0, 1), 0));
	float d = hiZSrc.Load(int3(index.xy * 2 + int2(1, 1), 0));
		
	hiZDst[index.xy] = min4(a.x, b.x, c.x, d.x);
}
